...
class ModelModuleFilterPro extends Model {
    ...
    LEFT JOIN " . DB_PREFIX . "product p ON(pov.product_id = p.product_id) ";
        ...
        $sql .= "LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON(p.product_id=p2s.product_id)
	WHERE ovd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND od.language_id = '" . (int)$this->config->get('config_language_id')."' ";
    ...
    public function getOptionValues($option_id) {
        ...
        $sql .= "WHERE ovd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND od.language_id = '" . (int)$this->config->get('config_language_id') . "' ";
        $sql .= "AND o.option_id = '".intval($option_id)."'";
	...
	public function getTotalProducts($data) {
        ...
        $option_range_filters = array();

        if($data['option_value_texts']) {
            foreach($data['option_value_texts'] as $option_id=>$option_value) {
                $option_range_filters[] = "(po.option_id = '" . $option_id . "' AND po.option_value = '".$this->db->escape($option_value)."')";
            }
        }

        if($data['option_value_min_ranges']) {
            foreach($data['option_value_min_ranges'] as $option_id=>$option_value) {
                $option_range_filters[] = "(po.option_id = '" . $option_id . "' AND po.option_value >= ".intval($option_value).")";
            }
        }

        if($data['option_value_max_ranges']) {
            foreach($data['option_value_max_ranges'] as $option_id=>$option_value) {
                $option_range_filters[] = "(po.option_id = '" . $option_id . "' AND po.option_value <= ".intval($option_value).")";
            }
        }

        if($option_range_filters) {
            if($this->filterpro_setting['option_mode'] == 'and') {
                //$sql .= " AND (" . implode(" AND ", $option_range_filters) . ")";

                foreach($option_range_filters as $i => $option_filter) {
                    $sql .= " AND EXISTS (select 1 FROM " . DB_PREFIX . "product_option po" . $i .
                        " WHERE  po" . $i . ".product_id=po.product_id AND ". str_replace('po.','po'.$i.'.',$option_filter). ") ";
                }
            } else {
                $sql .= " AND (" . implode(" OR ", $option_range_filters) . ")";
            }
        }
...
//			$sql .= " AND realprice <=" . ($max_price);
        ...
        $sql .= " AND special_price IS NOT NULL";
...
//			$sql .= " AND realprice <=" . ($max_price);
	...
	public function getTaxRates($tax_class_id) {
		...
		$query = $this->db->query("SELECT category_id FROM " . DB_PREFIX . "category WHERE parent_id = " . $category_id . " AND status = 1");
