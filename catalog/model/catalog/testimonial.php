...
class ModelCatalogTestimonial extends Model {
	...
	public function addTestimonial($data, $status) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "testimonial SET status = '".$status."', name='".$this->db->escape($data['name'])."', city='".$this->db->escape($data['city'])."', email='".$this->db->escape($data['email'])."', phone='".$this->db->escape($data['phone'])."', date_added = '" . $date . "'");
		...
		foreach ($results->rows as $result) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "testimonial_description SET testimonial_id = '" . (int)$testimonial_id . "', language_id = '".(int)$result['language_id']."', description = '" . $this->db->escape($data['description']) . "'");
		}
