...
class ControllerProductSearch extends Controller { 	
	public function index() {
        ...
        $search_all = !empty($this->request->get['search_all']) ? true : false;
...
//                $rub = $this->currency->format($this->tax->calculate($rub, $result['tax_class_id'], $this->config->get('config_tax')));
        ...
        if (count($this->data['products']) == 1 && !empty($this->data['products'][0]['href']) && empty($search_all)) {
            $this->response->redirect(html_entity_decode($this->data['products'][0]['href']));
        } else if (empty($search_all)) {
            $this->response->redirect('/search?filter_name='.$filter_name.'&search_all=1');
        } else {
