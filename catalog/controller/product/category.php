	...
	public function index() { 
		...
		if ($category_info) {
			...
			if ($category_info['image']) {
				...
				$this->data['thumb'] = $this->model_tool_object->resizeimage($category_info['image'], $this->config->get('config_image_category_width'), $this->config->get('config_image_category_height'));
                //$this->data['thumb'] = !empty($category_info['image']) ? '/image/'.$category_info['image'] : '';
			...
			/*Вывод категорий по районам для продажи*/
		    $area_sale = $this->model_catalog_category->getCategoryАrea(118);
			
		    $this->data['category_area_sale'] = $this->model_catalog_category->getCategoriesByParentId(118);
			
			
			foreach($area_sale as $categories_area){
			
		    $this->data['areas_sale'][] = array(
			
			        'cat_area'   =>  $categories_area["category_id"],
				
					'name'       =>  $categories_area["name"],
					
					'image'      =>  $categories_area["image"],
					
					'path'       =>  $this->url->link('product/category', 'path=' . $categories_area["category_id"] . $url),
					
					'href'       => $this->url->link('product/category', 'path=' . $this->request->get['path'] . '_' . $categories_area["category_id"] . $url),	
				);
			
			}
		    /*Вывод категорий по районам для продажи*/
			
		    /*Вывод категорий по районам для аренды*/
		    $area_rent = $this->model_catalog_category->getCategoryАrea(119);
			
			$this->data['category_area_rent'] = $this->model_catalog_category->getCategoriesByParentId(119);
			
			foreach($area_rent as $categories_area){
			
		    $this->data['areas_rent'][] = array(
			...
			foreach ($results as $result) {
				...
				//$image = $this->model_tool_object->resizeimage($result['image'], $this->config->get('config_image_category_width'), $this->config->get('config_image_category_height'));
                $image = !empty($result['image']) ? '/image/'.$result['image'] : '';
...
//                $rub = $this->currency->format($this->tax->calculate($rub, $result['tax_class_id'], $this->config->get('config_tax')));
            ...
            $this->data['pages_total'] = $pagination->total;
