...
class ControllerProductProduct extends Controller {
	...
	public function index() { 
		...
		if ($product_info) {
			...
			/*Подключение модели отображения обьекта без растягивания изображения*/
			...
			$plan_object = $this->model_catalog_product->getProductPlans($this->request->get['product_id']);
			
			foreach($plan_object as $plans){
				$this->data['plans'][] = array(
					'plan_object' => $this->model_tool_object->resizeimage($plans['image'], $this->config->get('config_image_popup_width'), $this->config->get('config_image_popup_height')),
				);
			}
			/*Вывод плана объекта*/
			...
			foreach ($results as $result) {
				$this->data['images'][] = array(
					'popup' => $this->model_tool_object->resizeimage($result['image'], $this->config->get('config_image_popup_width'), $this->config->get('config_image_popup_height')),
					'thumb' => $this->model_tool_object->resizeimage($result['image'], $this->config->get('config_image_additional_width'), $this->config->get('config_image_additional_height')),
				);
			}	
...
//            $this->data['rub'] = $this->currency->format($this->tax->calculate($rub, $product_info['tax_class_id'], $this->config->get('config_tax')));
			...
			$this->data['column_row'] = array();
			
			foreach($attribute_groups as $groups){
			$this->data['column_row'][] = array(
				'column' => $groups['column_row'],
			);
			}
			...
			/*Правки, добавление дополнительных данных о обьекте*/
			
			$this->data['get_option_required'] = $this->model_catalog_product->GetRequiredOptions($product_id);
			$this->data['get_attribute_value'] = $this->model_catalog_product->GetAttributsValue($product_id);
			$this->data['get_attribute'] = $this->model_catalog_product->getProductAttributes($product_id);
			$this->data['number_of_rooms_value'] = $this->model_catalog_product->getSelectionProductOption($product_id, 43);
			$this->data['number_of_beds_value'] = $this->model_catalog_product->getSelectionProductOption($product_id, 48);
			$this->data['column_size_3'] = '';
			$this->data['social_button'] = $this->config->get('social_button');
			
			/*Вывод фотографии агента*/
			$this->data['inf_agent'] = $this->model_catalog_product->GetInformationAgent($product_id);
			/*Вывод фотографии агента*/
			...
			if (isset($this->request->get['filter_category_id'])) {
      		...
      		$this->data['heading_title'] = $this->language->get('object_error');
    ...
    public function gallery() {
        ...
        if ($product_info) {
            ...
            $this->data['thumb_orig'] = $this->model_tool_image->resize($product_info['image'], $width_orig, $height_orig);
