...
class ControllerModuletestimonial extends Controller {
	...
	protected function index($setting) {
		...
		$this->data['show_testimonial'] = $this->language->get('show_testimonial');
		...
		$this->data['sent_url'] = $this->url->link('product/isitestimonial', '', 'SSL'); 
		...
		foreach ($results as $result) {
			...
			$result['description'] = trim(strip_tags($result['description']));
			$result['description'] = str_replace('«<p>', '«', $result['description']);
			$result['description'] = str_replace('</p>»', '»', $result['description']);

            $setting['testimonial_character_limit'] = 150;
			...
			if ($setting['testimonial_character_limit']>0)
			{
				...
				if (mb_strlen($result['description'],'UTF-8')>$lim) 
					$result['description'] = mb_substr($result['description'], 0, $lim-3, 'UTF-8'). ' ' .'<a href="'.$this->data['more']. $result['testimonial_id'] .'" title="'.$this->data['text_more2'].'">...</a>';
			...
			if (mb_strlen($result['name'],'UTF-8')>10)
					$result['name'] = mb_substr($result['name'], 0, 25, 'UTF-8').'';

			if (mb_strlen($result['city'],'UTF-8')>20)
