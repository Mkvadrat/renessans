...
<div class="heading">
      ...
      <div class="buttons"><a onclick="$('#form').submit();" class="button"><?php echo $button_save; ?></a><a onclick="sav_con()" class="button"><span><?php echo $button_save_continue; ?></span></a><a onclick="location = '<?php echo $cancel; ?>';" class="button"><?php echo $button_cancel; ?></a></div>
...
<div class="ionTabs" id="tabs_1" data-name="Tabs_Group_name">
    <ul class="ionTabs__head">
		...
		<?php if($getblock){ ?><li class="ionTabs__tab" data-target="tab-design"><?php echo $tab_design; ?></li><?php } ?>
    </ul>
    <div class="ionTabs__body">
        ...
        <div class="ionTabs__item" data-name="tab-data">
			...
			<input type="hidden" name="product_description[<?php echo $language['language_id']; ?>][meta_keyword]" value="" />
            ...
            <tr>
              ...
              <td><input type="text" name="ean" value="<?php echo $ean; ?>" />*Для пользования маркером поле Yandex-карта должно быть пустым!</td>
            </tr>
			<tr>
...
<div id="YMapsID" style="width:1170px; height:400px"></div>
        ...
        <table id="attribute" class="list">
            ...
            <tbody id="attribute-row<?php echo $attribute_row; ?>">
              <tr>
				...
				<!--Новое поле сортировка атрибутов-->
                <!--<td class="left">-->
				<input type="hidden" name="product_attribute[<?php echo $attribute_row; ?>][sort_attribute]" value="" />
				<?php foreach ($languages as $language) { ?>
				<input type="hidden" name="product_attribute[<?php echo $attribute_row; ?>][product_attribute_description][<?php echo $language['language_id']; ?>][text]" value="" />
                  <!--<textarea name="product_attribute[<?php echo $attribute_row; ?>][product_attribute_description][<?php echo $language['language_id']; ?>][text]" cols="40" rows="5">
				  ...
				  </textarea><img src="view/image/flags/<?php echo $language['image']; ?>" title="<?php echo $language['name']; ?>" align="top" /><br />-->
                <?php } ?>
				<!--</td>-->
				<!--Новое поле сортировка атрибутов-->				  
		 ...
		 <div id="tab-option">
		    ...
		    <select name="main_option_value" style="width: 192px;">
                ...
                <option value="1">Основные опции для квартиры, номера</option>
				<option value="2">Основные опции для дома, дачи</option>
				<option value="3">Основные опции для коммерч. недвижимости</option>
		...
		<?php } ?>
		 <div class="ionTabs__item" data-name="tab-image">
	...
	$.ajax({
		...
		success: function(data) {
			for (i = 0; i < data.length; i++) {
	 			$('#product').append('<option value="' + data[i]['product_id'] + '">' + 'ID=' + data[i]['model'] + ' - ' + data[i]['name'] + '</option>');
			}
		}
	});
}
...
function addAttribute() {
	...
	//html += '    <td class="left">';
	//html += '    <input type="hidden" name="product_attribute[' + attribute_row + '][sort_attribute]" value="" />';
	<?php foreach ($languages as $language) { ?>
	html += '    <input type="hidden" name="product_attribute[' + attribute_row + '][product_attribute_description][<?php echo $language['language_id']; ?>][text]" value="" />';
...
<!--Пока непонятное поле, за что оно отвечает-->
<!--<script type="text/javascript"><!--
...
function addMultiImage(img) {
            ...
            html ='<tbody id="image-row'+ image_row +'">';
              html+='<tr>';
                html+='<td class="left"><div class="image"><img src="<?php echo HTTP_CATALOG."image/"; ?>/'+img+'" alt="" id="thumb' + image_row + '" height=100 width=100 />';
