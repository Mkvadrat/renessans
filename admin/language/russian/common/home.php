<?php
// Heading
$_['heading_title']                	= 'Панель управления';

// Text
$_['text_settings']					= 'Настройка';
$_['text_modules']					= 'Модули';
$_['text_modules_shipping']			= 'Модули доставки';
$_['text_modules_payment']			= 'Модули оплаты';
$_['text_add_product']				= 'Добавить товар';
$_['text_products']					= 'Объекты';
$_['text_add_category']				= 'Добавить категорию';
$_['text_categories']				= 'Категории';
$_['text_attributes']				= 'Атрибуты';
$_['text_options']					= 'Опции';
$_['text_manufacturer']				= 'Производители';
$_['text_downloads']				= 'Загрузки';
$_['text_information']				= 'Статьи';
$_['text_reviews']					= 'Отзывы';
$_['text_orders']					= 'Заказы';
$_['text_returns']					= 'Возвраты';
$_['text_customers']				= 'Покупатели';
$_['text_coupons']					= 'Купоны';
$_['text_mail']						= 'Почта';
$_['text_purchased']				= 'Товарный отчет';
$_['text_backup_restore']			= 'Резервирование';
$_['text_overview']                	= 'Краткий обзор';
$_['text_statistics']              	= 'Статистика';
$_['text_latest_10_orders']        	= 'Последние 10 заказов';
$_['text_total_product']            = 'Всего товаров:';
$_['text_total_sale']              	= 'Всего продано на сумму:';
$_['text_total_sale_year']         	= 'Всего продано в этом году на сумму:';
$_['text_total_sales_previous_years'] = 'Общий объем продаж предыдущих лет:';
$_['text_total_review']             = 'Общее количество просмотров:';
$_['text_total_order']             	= 'Всего заказов:';
$_['text_abandoned_orders']         = 'Всего брошенных заказов за последние 30 дней:';
$_['text_total_customer']          	= 'Всего покупателей:';
$_['text_total_customer_approval'] 	= 'Покупателей, ожидающих подтверждения:';
$_['text_total_review_approval']   	= 'Отзывов, ожидающих подтверждения:';
$_['text_total_affiliate']          = 'Количество партнёров:';
$_['text_total_affiliate_approval'] = 'Партнёров, ожидающих подтверждения:';
$_['text_day']                     	= 'Последние 24 часа';
$_['text_week']                    	= 'Последние 7 дней';
$_['text_month']                   	= 'Последний месяц';
$_['text_year']                    	= 'Последний год';
$_['text_order']                   	= 'Всего заказов';
$_['text_customer']                	= 'Всего покупателей';
$_['text_quick_buttons']            = 'Кнопки быстрого доступа';
$_['text_clear_cache']              = 'Очистить кэш';
$_['text_info']              		= 'Статистика';
$_['text_close']              		= 'Закрыть';
$_['text_view']              		= 'Просмотр';

// Column 
$_['column_order']                 	= '№ заказа';
$_['column_customer']              	= 'Покупатель';
$_['column_status']                	= 'Статус';
$_['column_date_added']            	= 'Добавлено';
$_['column_total']                 	= 'Итого';
$_['column_firstname']             	= 'Имя';
$_['column_lastname']              	= 'Фамилия';
$_['column_action']                	= 'Действие';
$_['date_modify']                   = 'Дата изменения объекта';
$_['date_modified']                 = 'В объекте<br>изменения<br>отсутствуют';

// Column 2
$_['column_name']            = 'Название объекта';
$_['column_category']        = 'Категория';
$_['column_model']           = 'Модель';
$_['column_image']           = 'Изображение';
$_['column_price']           = 'Цена на сайте';
$_['column_quantity']        = 'Количество';
$_['column_status']          = 'Статус';
$_['column_action']          = 'Действие';
$_['date_modify']            = 'Дата изменения объекта';
$_['date_modified']          = 'В объекте<br>изменения<br>отсутствуют';

// Entry
$_['entry_range']                  	= 'Выбор диапазона:';

// Error
$_['error_install']                 = 'Установочная папка до сих пор существует!';
$_['error_image']                   = 'Папка с картинками %s недоступна для записи!';
$_['error_image_cache']             = 'Папка для закэшированных картинок %s недоступна для записи!';
$_['error_cache']                   = 'Папка кэша %s недоступна для записи!';
$_['error_download']                = 'Папка для загрузок %s недоступна для записи!';
$_['error_logs']                    = 'Папка для логов %s недоступна для записи!';
?>