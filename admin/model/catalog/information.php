...
class ModelCatalogInformation extends Model {
	public function addInformation($data) {
        ...
        if (!isset($data['show_in_blog'])) $data['show_in_blog'] = 0;
        if (!isset($data['blog_type'])) $data['blog_type'] = 0;
        if (!isset($data['blog_category'])) $data['blog_category'] = 0;
		/*Блог*/
	
		$this->db->query("INSERT INTO " . DB_PREFIX . "information SET sort_order = '" . (int)$data['sort_order'] . "', bottom = '" . (isset($data['bottom']) ? (int)$data['bottom'] : 0) . "', status = '" . (int)$data['status'] . "', /*Чистый шаблон в статьях, без заголовка и breadcrumbs*/ clean = '" . (isset($data['clean']) ? (int)$data['clean'] : 0) . "' /*Чистый шаблон в статьях, без заголовка и breadcrumbs*/, /*Блог*/ image = '".$this->db->escape($data['image'])."', show_in_blog = '".$this->db->escape($data['show_in_blog'])."', blog_type = '".$this->db->escape($data['blog_type'])."', blog_category = '".$this->db->escape($data['blog_category'])."'/*Блог*/");
	...
	public function editInformation($information_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "information SET sort_order = '" . (int)$data['sort_order'] . "', bottom = '" . (isset($data['bottom']) ? (int)$data['bottom'] : 0) . "', status = '" . (int)$data['status'] . "', /*Чистый шаблон в статьях, без заголовка и breadcrumbs*/ clean = '" . (isset($data['clean']) ? (int)$data['clean'] : 0) . "' /*Чистый шаблон в статьях, без заголовка и breadcrumbs*/, /*Блог*/ image = '".$this->db->escape($data['image'])."', show_in_blog = '".$this->db->escape($data['show_in_blog'])."', blog_type = '".$this->db->escape($data['blog_type'])."', blog_category = '".$this->db->escape($data['blog_category'])."' /*Блог*/ WHERE information_id = '" . (int)$information_id . "'");
