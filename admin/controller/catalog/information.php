...
class ControllerCatalogInformation extends Controller { 
	...
	private function getForm() {
		$this->data['heading_title'] = $this->language->get('heading_title');
		$this->data['heading_title_info'] = $this->language->get('heading_title_info');
		...
		$this->data['info_bottom'] = $this->language->get('info_bottom');
		...
		/*Чистый шаблон в статьях, без заголовка и breadcrumbs*/
		if (isset($this->request->post['clean'])) {
            $this->data['clean'] = $this->request->post['clean'];
        } elseif (!empty($information_info)) {
            $this->data['clean'] = $information_info['clean'];
        } else {
            $this->data['clean'] = 0;
        }
		/*Чистый шаблон в статьях, без заголовка и breadcrumbs*/
        ...
        } else if (!empty($information_info['image']) && file_exists(DIR_IMAGE . $information_info['image'])) {
            $image = $information_info['image'];
        } else {
            $image = 'no_image.jpg';
        }
        $this->data['information_image'] = array(
            'image'      => $image,
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            $this->data['show_in_blog'] = !empty($this->request->post['show_in_blog']) ? 1 : 0;
        } elseif (!empty($information_info['show_in_blog'])) {
            $this->data['show_in_blog'] = 1;
        } else {
            $this->data['show_in_blog'] = 0;
        }

        if (isset($this->request->post['blog_type'])) {
            $this->data['blog_type'] = $this->request->post['blog_type'];
        } else if (!empty($information_info['blog_type'])) {
            $this->data['blog_type'] = $information_info['blog_type'];
        } else {
            $this->data['blog_type'] = 0;
        }

        if (isset($this->request->post['blog_category'])) {
            $this->data['blog_category'] = $this->request->post['blog_category'];
        } else if (!empty($information_info['blog_category'])) {
            $this->data['blog_category'] = $information_info['blog_category'];
        } else {
            $this->data['blog_category'] = 0;
        }
