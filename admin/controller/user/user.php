...
class ControllerUserUser extends Controller {  
	private $error = array();
  	...
  	public function update() {
		...
		/*Поле скайп, изображения и дополнительных телефонов в форме пользователей*/
		$this->load->model('tool/image');
		
		if (isset($this->request->post['image']) && file_exists(DIR_IMAGE . $this->request->post['image'])) {
			$this->data['thumb'] = $this->model_tool_image->resize($this->request->post['image'], 100, 100);
		} elseif (!empty($user_info['image']) && file_exists(DIR_IMAGE . $user_info['image'])) {
		...
		if (!empty($_FILES['image']['tmp_name'])) {
			...
			$file_2 = $this->model_tool_image->resize($fileName, 100, 100);
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			/*Присутсвует переменная, для сохрания фотографии*/
			$this->model_user_user->editUser($this->request->get['user_id'], $this->request->post, $this->request->post['image']);
			/*Присутсвует переменная, для сохрания фотографии*/
			
			/*Оригинальное обращение к модели с ocstore? раскоментировать при удалении фрагмента выше*/
	...
	private function getForm() {
		...
		$this->data['text_image_clear'] = $this->language->get('text_image_clear');
